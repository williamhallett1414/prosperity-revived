...
const DIFFICULTY_STYLES = {
  ...
  advanced:     'bg-[#0A1A2F]/8 text-[#0A1A2F] font-bold',
};
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto bg-white border-[#BAE6FD]/30">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl text-[#0A1A2F]">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {workout?.difficulty && (
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ${DIFFICULTY_STYLES[workout.difficulty] || 'bg-[#BAE6FD]/20 text-[#0A1A2F]/60'}`}>
                  {workout.difficulty}
                </span>
              )}
              <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-[#BAE6FD]/15 text-[#0A1A2F]/60 flex items-center gap-1">
            ...
            <p className="text-sm text-[#0A1A2F]/60 leading-relaxed">{workout?.description}</p>

            {/* Exercise list */}
            {workout?.exercises?.length > 0 && (
              <div>
                <h3 className="font-semibold text-sm text-[#0A1A2F] mb-2.5 flex items-center gap-2">
                  ...
                  {workout.exercises.map((ex, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 bg-[#F0F8FF] rounded-xl border border-[#BAE6FD]/25">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-sm text-[#0A1A2F] truncate">{ex.name}</p>
