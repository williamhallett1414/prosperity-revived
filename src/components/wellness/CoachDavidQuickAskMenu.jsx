      ...
      <div className="px-4 pb-3 grid grid-cols-2 gap-2">
        {quickAsks.map((item, idx) => (
          <motion.button
            ...
            disabled={isLoading}
            className="text-xs px-3 py-2 rounded-lg bg-white dark:bg-white/5 hover:bg-gray-100 dark:bg-white/5 text-[#0A1A2F] dark:text-white border border-[#F2F6FA] transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-left font-medium"
          >
            {item.label}
          </motion.button>
        ))}
