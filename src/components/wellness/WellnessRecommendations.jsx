...
export default function WellnessRecommendations({ user }) {
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <Link to={createPageUrl('DiscoverRecipes')}>
          <div className="bg-gradient-to-br from-[#FAD98D] to-[#AFC7E3] rounded-2xl p-5 text-[#0A1A2F] dark:text-white cursor-pointer hover:shadow-lg transition-shadow">
            ...
            <p className="text-[#0A1A2F]/70 dark:text-white/70 text-sm">Browse and create delicious recipes</p>
          </div>
        </Link>
      </motion.div>
      ...
      >
        <h3 className="text-lg font-bold text-[#0A1A2F] dark:text-white mb-3">New Workouts to Try</h3>
      ...
      >
        <h3 className="text-lg font-bold text-[#0A1A2F] dark:text-white mb-3">Workout Challenges</h3>
              ...
              >
                <div className="flex flex-col items-center text-center gap-2">
                  ...
                  <div className="w-full">
                    <h4 className="font-bold text-[#0A1A2F] dark:text-white text-xs mb-1 line-clamp-2">{challenge.title}</h4>
                    <p className="text-[10px] text-[#0A1A2F]/60 dark:text-white/60 mb-2">{challenge.duration_days} Days</p>
                    {isParticipating ? (
                      <div className="w-full">
                        <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                          <div className="h-full bg-[#FAD98D] rounded-full transition-all" style={{ width: `${progress}%` }} />
                        </div>
                        <p className="text-[10px] text-[#0A1A2F]/60 dark:text-white/60 mt-1">{progress}%</p>
                      </div>
                    ) : (
