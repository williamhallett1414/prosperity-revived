...
function WorkoutMiniCard({ workout, onRemove }) {
  ...
  return (
    <div className={`flex items-center gap-2.5 p-2.5 rounded-xl bg-white dark:bg-white/5 border-l-4 shadow-sm dark:shadow-none group`}
    style={{ borderLeftColor: meta.bar.replace('bg-', '').startsWith('#') ? meta.bar.replace('bg-', '') : undefined }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-gray-400 dark:text-gray-300">{workout.duration_minutes} min · {workout.exercises?.length || 0} exercises</p>
      </div>
      {onRemove &&
...
function AddWorkoutSheet({ dayIdx, onClose, onAdd }) {
  ...
  return (
    ...
    onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div className="bg-gray-50 dark:bg-white/5 dark:text-white rounded-t-3xl w-full max-w-lg max-h-[92vh] flex flex-col overflow-hidden shadow-2xl relative">
        {/* Header */}
        ...
        <div className="px-4 pt-4 pb-3 space-y-3 flex-shrink-0 bg-gray-50 dark:bg-white/5 border-b border-gray-200 dark:border-white/10">
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-none">
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all
                  ${catFilter === cat ? 'bg-[#FD9C2D] text-white border-[#FD9C2D]' : 'bg-white dark:bg-white/5 text-gray-500 dark:text-gray-300 border-gray-200 dark:border-white/10 hover:border-[#FD9C2D]/50'}`}>
          ...
          {filtered.map((w) => {
            ...
            return (
              ...
              className={`bg-white dark:bg-white/5 rounded-2xl border-2 overflow-hidden transition-all cursor-pointer
                  ${isSelected ? 'border-[#FD9C2D] shadow-md dark:shadow-none shadow-[#FD9C2D]/10' : 'border-transparent hover:border-gray-200 dark:border-white/10'}`}
              onClick={() => {setSelected(isSelected ? null : w);setExpanded(isExpanded ? null : w.id);}}>
                ...
                <div className="flex items-center gap-3 p-3.5">
                  ...
                  <ChevronRight className="w-4 h-4 text-gray-300 dark:text-gray-400 dark:text-gray-300 flex-shrink-0" />
                  }
                </div>
                ...
                <div className="px-4 pb-3 border-t border-gray-50 dark:border-white/5">
                    <p className="text-[10px] text-gray-400 dark:text-gray-300 font-semibold uppercase tracking-wide my-2">Exercises</p>
                    <div className="grid grid-cols-2 gap-1.5 mb-3">
                      {(w.exercises || []).map((ex, i) =>
                    <div key={i} className="bg-gray-50 dark:bg-white/5 dark:text-white rounded-lg px-2 py-1.5">
        ...
        <div className="p-4 bg-white dark:bg-white/5 border-t border-gray-200 dark:border-white/10 shadow-2xl shadow-black/10 rounded-br-3xl flex-shrink-0">
          ...
          className={`w-full py-3.5 rounded-2xl font-black text-base transition-all
              ${selected ?
          'bg-[#FD9C2D] text-white shadow-lg dark:shadow-none shadow-[#FD9C2D]/40 active:scale-95' :
    ...
    <div className="flex-1">
      ...
      <div className="space-y-2.5">
        ...
        <WorkoutMiniCard key={w.wid} workout={w} onRemove={() => removeWorkout(w.wid)} />
          ...
          <button onClick={onAddWorkout}
        className="w-full py-2.5 rounded-xl border-2 border-dashed border-gray-200 dark:border-white/10 text-gray-400 dark:text-gray-300 text-xs font-semibold hover:border-[#FD9C2D]/40 hover:text-[#FD9C2D]/60 transition-all">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {stats.map((s) =>
        <div key={s.label} className="bg-white dark:bg-white/5 rounded-2xl p-4 text-center shadow-sm dark:shadow-none">
            <div className={`flex justify-center mb-1 ${s.color}`}>{s.icon}</div>
            <div className="text-2xl font-black text-[#3C4E53]">{s.value}</div>
            <div className="text-[11px] text-gray-400 dark:text-gray-300 font-semibold uppercase tracking-wide">{s.label}</div>
          </div>
        )}
      </div>

      {/* Mini calendar */}
      <div className="bg-white dark:bg-white/5 rounded-2xl p-4 shadow-sm dark:shadow-none">
          ...
          {week.map(({ dayIdx }) => {
            ...
            return (
              ...
              className={`rounded-xl p-2 text-center transition-colors hover:bg-gray-50 dark:bg-white/5 ${isToday ? 'ring-2 ring-[#FD9C2D]' : ''}`}>
                ...
                <p className="text-[9px] text-gray-400 dark:text-gray-300 font-bold uppercase mb-1.5">{DAYS_SHORT[dayIdx]}</p>
                {dayWorkouts.length === 0 ?
                <div className="h-7 rounded-lg bg-gray-100 dark:bg-white/5 flex items-center justify-center">
                    <span className="text-gray-300 dark:text-gray-400 dark:text-gray-300 text-[10px]">—</span>
                  </div> :
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-[8px] text-gray-400 dark:text-gray-300 text-center">+{dayWorkouts.length - 2}</p>
                  }
                  </div>
                }
              </button>);
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <span key={key} className="flex items-center gap-1 text-[10px] text-gray-400 dark:text-gray-300">
      ...
      <div className="bg-white dark:bg-white/5 rounded-2xl p-4 shadow-sm dark:shadow-none">
          ...
          <div className="space-y-1.5">
            ...
            return (
              <div key={idx}>
                  <p className="text-[10px] text-gray-400 dark:text-gray-300 font-bold uppercase tracking-wide mb-1">{day}</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
        ...
        <button key={cat} onClick={() => setCatFilter(cat)}
        className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all
              ${catFilter === cat ? 'bg-[#FD9C2D] text-white border-[#FD9C2D]' : 'bg-white dark:bg-white/5 text-gray-500 dark:text-gray-300 border-gray-200 dark:border-white/10'}`}>
      ...
      <div className="space-y-3">
        {filtered.map((w) => {
          ...
          const diff = DIFF_META[w.difficulty] || DIFF_META.beginner;
          return (
            <div key={w.id} className="bg-white dark:bg-white/5 rounded-2xl p-4 shadow-sm dark:shadow-none">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md ${diff.color}`}>{diff.label}</span>
                    <span className="text-[10px] text-gray-400 dark:text-gray-300"><Clock className="inline w-2.5 h-2.5" /> {w.duration_minutes}m</span>
                  </div>
                </div>
              </div>
              {w.description && <p className="text-xs text-gray-500 dark:text-gray-300 mb-3 leading-relaxed">{w.description}</p>}
              <div className="grid grid-cols-2 gap-1.5">
                {(w.exercises || []).map((ex, i) =>
                <div key={i} className="bg-gray-50 dark:bg-white/5 dark:text-white rounded-lg px-2.5 py-2">
                    <p className="text-xs font-semibold text-[#3C4E53] leading-tight">{ex.name}</p>
      ...
      <div className="max-w-2xl mx-auto px-4">
        <div className="bg-white dark:bg-white/5 rounded-2xl p-1 flex gap-1 shadow-sm dark:shadow-none mb-5">
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-bold transition-all
                ${view === n.key ? 'bg-[#FD9C2D] text-white shadow-sm dark:shadow-none' : 'text-gray-400 dark:text-gray-300 hover:text-gray-600 dark:text-gray-300'}`}>
            ...
            <div className="flex gap-2 overflow-x-auto pb-3 scrollbar-none mb-5">
              ...
              return (
                ...
                'bg-white dark:bg-white/5 border-gray-100 dark:border-white/10 hover:border-gray-200 dark:border-white/10'}`
                    ...
                    <span className={`text-[9px] font-bold uppercase tracking-tight ${isSelected ? 'text-white' : 'text-gray-500 dark:text-gray-300'}`}>
                      {DAYS_SHORT[dayIdx]}
                    </span>
