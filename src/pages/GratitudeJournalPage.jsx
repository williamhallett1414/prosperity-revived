...
const MOOD_CONFIG = {
  joyful:    { color: 'text-amber-600',  bg: 'bg-amber-50 dark:bg-amber-900/20',  border: 'border-amber-200 dark:border-amber-800/30',  dot: '#f59e0b', emoji: '😊' },
  grateful:  { color: 'text-green-600',  bg: 'bg-green-50 dark:bg-green-900/20',  border: 'border-green-200 dark:border-green-800/30',  dot: '#22c55e', emoji: '🙏' },
  hopeful:   { color: 'text-blue-600',   bg: 'bg-blue-50',   border: 'border-blue-200 dark:border-blue-800/30',   dot: '#3b82f6', emoji: '🌟' },
  peaceful:  { color: 'text-teal-600',   bg: 'bg-teal-50 dark:bg-teal-900/20',   border: 'border-teal-200',   dot: '#14b8a6', emoji: '😌' },
  struggling:{ color: 'text-red-500',    bg: 'bg-red-50 dark:bg-red-900/20',    border: 'border-red-200',    dot: '#f87171', emoji: '😔' },
  seeking:   { color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-900/20', border: 'border-purple-200 dark:border-purple-800/30', dot: '#a855f7', emoji: '💭' },
};

function getMoodStyle(mood) {
  return MOOD_CONFIG[mood] || { color: 'text-[#FAD98D]', bg: 'bg-white dark:bg-white/5', border: 'border-[#FAD98D]/30 dark:border-[#FAD98D]/10 dark:border-[#FAD98D]/5', dot: '#FAD98D', emoji: '✨' };
}
...
function WeekStrip({ days, streak }) {
  return (
    <div className="bg-white dark:bg-white/5 rounded-2xl border border-[#FAD98D]/20 dark:border-[#FAD98D]/10 dark:border-[#FAD98D]/5 p-4">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        {loadingReflection ? (
          <div className="space-y-2">
            <div className="h-3 bg-[#FAD98D]/15 dark:bg-[#FAD98D]/8 rounded-full w-full animate-pulse" />
            <div className="h-3 bg-[#FAD98D]/15 dark:bg-[#FAD98D]/8 rounded-full w-3/4 animate-pulse" />
          </div>
        ) : (
...
function GratitudeJournalPageInner() {
  ...
  return (
    <div className="min-h-screen bg-[#F2F6FA] dark:bg-[#0A1A2F] pb-28">
      ...
      {/* ── Header ── */}
      <div className="sticky top-0 z-40 bg-white dark:bg-white/5 border-b border-[#FAD98D]/20 dark:border-[#FAD98D]/10 dark:border-[#FAD98D]/5 px-4 py-3">
      ...
      <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
        ...
        <AnimatePresence mode="wait">
          ...
          <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            ...
            {/* AI Prompt */}
            <div className="bg-white dark:bg-white/5 rounded-2xl border border-[#FAD98D]/25 dark:border-[#FAD98D]/10 dark:border-[#FAD98D]/5 p-4">
              ...
              {loadingPrompt ? (
                <div className="space-y-1.5 mt-2">
                  <div className="h-3 bg-[#FAD98D]/15 dark:bg-[#FAD98D]/8 rounded-full w-full animate-pulse" />
                  <div className="h-3 bg-[#FAD98D]/15 dark:bg-[#FAD98D]/8 rounded-full w-2/3 animate-pulse" />
                </div>
              ) : (
          ...
          <div>
            ...
            {entries.length > 3 && (
              <button onClick={() => setShowAllHistory(s => !s)}
                className="w-full mt-3 py-2.5 rounded-xl bg-white dark:bg-white/5 border border-[#FAD98D]/20 dark:border-[#FAD98D]/10 dark:border-[#FAD98D]/5 text-xs font-bold text-[#0A1A2F]/40 dark:text-white/40 hover:text-[#0A1A2F]/70 dark:text-white/70 hover:border-[#FAD98D]/40 dark:border-[#FAD98D]/15 dark:border-[#FAD98D]/8 transition-all flex items-center justify-center gap-1.5">
                {showAllHistory
        ...
        {entries.length === 0 && !done && (
          <div className="bg-white dark:bg-white/5 rounded-2xl border border-[#FAD98D]/15 dark:border-[#FAD98D]/8 p-8 text-center">
