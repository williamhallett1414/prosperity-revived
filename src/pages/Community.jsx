...
import {
  ...
  Flame, ChevronRight, UserPlus, Crown, Edit3 } from
'lucide-react';
...
function StatPill({ icon: Icon, value, label, color }) {
  return (
    <div className="bg-white dark:bg-white/5 rounded-2xl px-4 py-5 border border-gray-100 dark:border-white/10 shadow-sm dark:shadow-none flex flex-col items-center justify-center gap-1.5 min-w-0">
      <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0"
      style={{ background: color + '22' }}>
        <Icon className="w-4 h-4" style={{ color }} />
      </div>
      <p className="text-2xl font-bold text-[#0A1A2F] dark:text-white leading-none">{value}</p>
      <p className="text-[10px] font-semibold text-[#0A1A2F]/40 dark:text-white/40 uppercase tracking-widest">{label}</p>
    </div>);
...
function CommunityInner() {
  ...
  return (
    <div className="min-h-screen bg-[#F2F6FA] dark:bg-[#0A1A2F] pb-28">
      ...
      <div className="sticky top-14 z-30 bg-[#F2F6FA]/95 dark:bg-[#0A1A2F]/95 backdrop-blur-sm border-b border-gray-100 dark:border-white/8 -mt-14">
        <div className="max-w-lg mx-auto px-4 py-2 flex items-center gap-1 overflow-x-auto scrollbar-hide">
          {TABS.map((tab) => {
            ...
            return (
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2 text-sm font-semibold flex-shrink-0 rounded-xl transition-all ${
              isActive ?
        ...
        className="mb-4">
            <Link to={createPageUrl('Friends')} className="block">
              <div className="bg-white dark:bg-white/5 rounded-2xl p-4 border border-gray-100 dark:border-white/10 shadow-sm dark:shadow-none hover:shadow-md dark:hover:shadow-none hover:border-[#EC4899]/30 dark:hover:border-[#EC4899]/20 transition-all flex items-center gap-3 cursor-pointer">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#EC4899] to-[#F472B6] flex items-center justify-center flex-shrink-0 shadow-sm">
                  <UserPlus className="w-6 h-6 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-base font-bold text-[#0A1A2F] dark:text-white">Find Friends</p>
                  <p className="text-sm text-[#0A1A2F]/50 dark:text-white/50">Connect with others on the same journey</p>
                </div>
