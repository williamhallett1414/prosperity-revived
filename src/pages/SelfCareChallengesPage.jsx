...
const FONT_LINK = "https://fonts.googleapis.com/css2?family=Lora:ital,wght@0,400;0,600;0,700;1,400&family=Nunito:wght@400;500;600;700;800;900&display=swap";
...
function getTotalXP(localData) {
  return CHALLENGES.reduce((sum, c) => {
    const days = getCompletedDays(localData[c.id]);
    return sum + days.length * c.xpPerDay;
  }, 0);
}
    ...
    <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:9999,overflow:"hidden"}}>
      {pieces.map(i => (
        <div key={i} style={{
          ...
          animation: `confettiFall ${1.5+(i%3)*0.4}s ease-in ${(i*0.04)}s forwards`,
          transform: `rotate(${i*37}deg)`,
        }} />
      ))}
      <style>{`@keyframes confettiFall { to { transform: translateY(100vh) rotate(${720}deg); opacity:0; } }`}</style>
    </div>
  );
}
...
// ─── Journey Map (day path visual) ───────────────────────────────────────────
function JourneyMap({ challenge, completedDays }) {
  const isDone = n => completedDays.includes(n);
  const isCurrent = n => !isDone(n) && (n === completedDays.length+1);
  return (
    <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap",padding:"8px 0"}}>
      {challenge.tasks.map((t,i) => {
        const done = isDone(t.day); const current = isCurrent(t.day); const locked = !done && !current;
        return (
...
function ChallengeDetail({ challenge, localData, onClose, onStart, onComplete, onReset }) {
  ...
  const handleComplete = async () => {
    ...
    if (isLast) { setShowConfetti(true); setTimeout(()=>setShowConfetti(false), 3000); }
  };
  ...
  return (
    <div style={{
      ...
      <XPToast show={showXP} xp={challenge.xpPerDay + (streak>=3?10:0)} />
      ...
      <div style={{background:"white", borderBottom:"1px solid #E8EDF3", padding:"14px 16px", display:"flex", alignItems:"center", gap:12, flexShrink:0}}>
        <button onClick={onClose} style={{
          width:38, height:38, borderRadius:"50%", background:"#F8F4EE",
          border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center",
          fontSize:18, color:"#0A1A2F"
        }}>←</button>
        <div style={{flex:1}}>
        ...
        <div style={{
          background:"linear-gradient(135deg,#D9B878,#c9a227)", borderRadius:14,
          padding:"4px 10px", fontSize:11, fontWeight:900, color:"#0A1A2F"
        }}>+{challenge.xpPerDay} XP/day</div>
        {isStarted && (
          <button onClick={()=>setConfirmReset(true)} style={{
            width:38, height:38, borderRadius:"50%", background:"#F8F4EE",
            border:"none", cursor:"pointer", fontSize:16, color:"#94a3b8"
          }}>↺</button>
        )}
      </div>

      <div className="detail-scroll" style={{flex:1, overflowY:"auto", paddingBottom:32}}>
        {/* Hero card */}
        <div style={{margin:"16px 16px 0", borderRadius:24, padding:20, background:challenge.bg, boxShadow:"0 12px 40px rgba(0,0,0,0.15)"}}>
          ...
          {isDone && (
            <div style={{background:"rgba(255,255,255,0.2)", borderRadius:16, padding:20, textAlign:"center"}}>
              ...
              <div style={{color:"rgba(255,255,255,0.7)", fontSize:12, marginTop:4}}>You finished all {challenge.duration} days · {challenge.xpPerDay * challenge.duration} XP earned</div>
            </div>
          )}
        </div>
        ...
        {isDone && (
          <div style={{margin:"16px 16px 0", display:"flex", flexDirection:"column", gap:10}}>
            <div style={{background:"white", borderRadius:20, border:"1px solid #E8EDF3", padding:"16px 20px"}}>
              ...
              {CHALLENGES.filter(c => c.id !== challenge.id && !localData[c.id]).slice(0,2).map(next => (
                <button key={next.id} style={{
        ...
        {isStarted && !isDone && currentTask && (
          <div style={{margin:"16px 16px 0"}}>
            ...
            {alreadyToday ? (
              ...
              <div style={{
                background:"white", borderRadius:24, border:`2px solid ${challenge.color}44`,
                padding:20, boxShadow:`0 4px 24px ${challenge.color}15`
              }}>
                {/* Day badge */}
                ...
                {/* Task content */}
                <div style={{
                  background:"#F8F4EE", borderRadius:14, padding:"12px 14px",
                  fontSize:13, color:"#0A1A2F88", lineHeight:1.7, marginBottom:16,
                }}>
                  {currentTask.content}
                </div>
                ...
                <div style={{marginBottom:8}}>
                  ...
                  <div style={{
                    background:"#F0EBE0", borderRadius:12, padding:"8px 12px",
                    fontSize:11, color:challenge.color, fontWeight:700, marginBottom:8,
                    fontStyle:"italic",
                  }}>
                  ...
                  <div style={{position:"relative"}}>
                    ...
                    <div style={{
                      ...
                      fontSize:10, color:charCount > 20 ? "#22c55e" : "#94a3b8", fontWeight:700
                    }}>{charCount}/∞</div>
                  </div>
                </div>
                ...
                {streak >= 3 && (
                  <div style={{
                    ...
                    padding:"8px 12px", fontSize:11, fontWeight:800, color:"#92400e",
                    marginBottom:12, display:"flex", alignItems:"center", gap:6,
                  }}>
                    🔥 {streak}-day streak! Bonus +10 XP on completion
                  </div>
                )}
                ...
                <button onClick={handleComplete} disabled={saving || !reflection.trim()} style={{
                  ...
                  {saving ? (
                    ...
                    <>✓ Complete Day {nextDayNum} · +{challenge.xpPerDay + (streak>=3?10:0)} XP</>
                  )}
                </button>
              </div>
            )}
        ...
        <div style={{margin:"20px 16px 0"}}>
          <button onClick={()=>setShowAllDays(s=>!s)} style={{
            ...
            <span style={{color:"#0A1A2F33", fontSize:16, transform: showAllDays?"rotate(180deg)":"none", transition:"0.3s"}}>⌄</span>
          </button>
...
function ChallengeCard({ challenge, localData, onOpen }) {
  ...
  return (
    <button onClick={()=>onOpen(challenge)} style={{
        ...
        <div style={{display:"flex", alignItems:"flex-start", gap:12}}>
          ...
          <div style={{flex:1, minWidth:0}}>
            ...
            <div style={{display:"flex", flexWrap:"wrap", gap:5, margin:"5px 0"}}>
              ...
              {doneToday && <span style={{fontSize:10,fontWeight:800,color:"#22c55e"}}>✓ Done</span>}
        ...
        <div style={{position:"sticky",top:0,zIndex:40,background:"rgba(248,244,238,0.92)",backdropFilter:"blur(12px)",borderBottom:"1px solid #E8EDF3",padding:"12px 16px"}}>
          <div style={{maxWidth:640,margin:"0 auto",display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{
              ...
              boxShadow:"0 4px 12px rgba(217,184,120,0.3)", cursor:"pointer",
            }}>
        ...
        <div style={{maxWidth:640,margin:"0 auto",padding:"16px 16px 0"}}>
          ...
          {hasAny && (
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:20}}>
              {[
                { label:"Active", value:activeList.length, emoji:"🔥", color:"#D9A84A" },
                { label:"Done", value:completedCount, emoji:"🏆", color:"#22c55e" },
                { label:"Level", value:level.label, emoji:level.emoji, color:"#8b5cf6" },
              ].map(s => (
          ...
          {!hasAny && (
            <div style={{
              background:"linear-gradient(135deg,#0A1A2F,#1a3a5c)", borderRadius:24,
              ...
              <div style={{color:"rgba(255,255,255,0.7)",fontSize:13,lineHeight:1.6,marginBottom:14}}>
                Each challenge is designed to build one positive habit at a time — for your faith, mind, body, and relationships.
              </div>
          ...
          <div className="sc-scroll" style={{display:"flex",gap:8,overflowX:"auto",marginBottom:16,paddingBottom:4}}>
            {CATS.map(cat => (
              <button key={cat} className="cat-btn" onClick={()=>setActiveCat(cat)} style={{
                ...
                color: activeCat===cat ? "white" : "#0A1A2F66",
                cursor:"pointer",
              }}>{cat}</button>
            ))}
          </div>
          ...
          {/* Bottom nudge if no active */}
          {hasAny && activeList.length === 0 && completedCount > 0 && (
            <div style={{
              marginTop:24, background:"white", borderRadius:22, border:"1px solid #E8EDF3",
