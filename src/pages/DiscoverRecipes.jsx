...
import { UtensilsCrossed, Plus, Sparkles, TrendingUp, Users, BookOpen, ShoppingCart } from 'lucide-react';
...
function EmptyState({ icon, title, sub, action, onAction }) {
  return (
    <div className="text-center py-12">
      <div className="w-16 h-16 rounded-2xl bg-[#F2F6FA] flex items-center justify-center mx-auto mb-3">{icon}</div>
      <p className="font-bold text-[#0A1A2F]/60 text-sm">{title}</p>
      {sub    && <p className="text-xs text-[#0A1A2F]/35 mt-1 mb-4">{sub}</p>}
      {action && <button onClick={onAction} className="text-xs font-bold text-[#c9a227] hover:opacity-70 transition-opacity">{action}</button>}
    </div>
  );
}
...
export default function DiscoverRecipes() {
  ...
  const [filters,    setFilters]    = useState({ search: '', dietType: 'all', category: 'all', prepTime: 'all' });
  ...
  const popularRecipes   = [...communityRecipes].sort((a, b) => (b.likes || 0) - (a.likes || 0)).slice(0, 8);
  ...
  const applyFilters = (list) => {
    ...
    return list.filter(r => {
      if (filters.search) {
        ...
        if (!r.title?.toLowerCase().includes(q) && !r.ingredients?.some(i => i.toLowerCase().includes(q))) return false;
      }
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF7] pb-28">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setCartOpen(true)}
                ...
                {totalCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-4.5 h-4.5 bg-[#c9a227] text-white text-[9px] font-bold rounded-full flex items-center justify-center"
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-4">
        ...
        {activeTab === 'browse' && (
          <>
            ...
            {browseSrc.length === 0
              ? <EmptyState icon={<UtensilsCrossed className="w-8 h-8 text-[#0A1A2F]/15" />} title="No recipes match" sub="Try adjusting your filters" action="Clear filters" onAction={clearFilters} />
              : <>
                  <p className="text-[10px] font-bold text-[#0A1A2F]/35 uppercase tracking-widest">
                    {isFiltered(filters) ? `Results · ${browseSrc.length} found` : `Popular · ${browseSrc.length} recipes`}
                  </p>
                  <div className="space-y-3">{browseSrc.map((r, i) => <RecipeCard key={r.id} recipe={r} index={i} />)}</div>
                </>
            }
          </>
        )}
        ...
        {activeTab === 'mine' && (
          myRecipes.length === 0
            ? <EmptyState icon={<BookOpen className="w-8 h-8 text-[#0A1A2F]/15" />} title="No recipes yet" sub="Add your first recipe to build your personal cookbook" action="Add Recipe" onAction={() => setShowCreate(true)} />
            : mineSrc.length === 0
              ? <EmptyState icon={<UtensilsCrossed className="w-8 h-8 text-[#0A1A2F]/15" />} title="No matches" sub="None of your recipes match this filter" action="Clear filters" onAction={clearFilters} />
              : <div className="space-y-3">{mineSrc.map((r, i) => <RecipeCard key={r.id} recipe={r} index={i} />)}</div>
        )}

        {activeTab === 'community' && (
          communityRecipes.length === 0
            ? <EmptyState icon={<Users className="w-8 h-8 text-[#0A1A2F]/15" />} title="No shared recipes yet" sub="Share your recipes so others can discover them" action="Add Recipe" onAction={() => setShowCreate(true)} />
            : communitySrc.length === 0
              ? <EmptyState icon={<UtensilsCrossed className="w-8 h-8 text-[#0A1A2F]/15" />} title="No matches" sub="Try adjusting your filters" action="Clear filters" onAction={clearFilters} />
              : <div className="space-y-3">{communitySrc.map((r, i) => <RecipeCard key={r.id} recipe={r} index={i} />)}</div>
        )}

        {activeTab === 'collections' && <RecipeCollections allRecipes={recipes} />}
      </div>
      ...
      <ChefDaniel user={user} userRecipes={myRecipes} mealLogs={mealLogs}
